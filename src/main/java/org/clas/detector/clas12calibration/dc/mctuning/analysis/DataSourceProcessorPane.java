package org.clas.detector.clas12calibration.dc.mctuning.analysis;
...
import org.clas.detector.clas12calibration.dc.calt2d.CalUtility;
...
public class DataSourceProcessorPane extends JPanel implements ActionListener {
    ...
    public boolean               isHipo4Event  = false;          
    ...
    private void initUI(){
        ...
        JButton sourceFileHipo4L = new JButton("H4 list");
        sourceFileHipo4L.setActionCommand("OpenFileHipoL");
        sourceFileHipo4L.setToolTipText("Open Hipo4 files");
        sourceFileHipo4L.addActionListener(this);
        ...
        sourcePane.add(sourceFileHipo4L);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getActionCommand().compareTo("OpenFileHipoL")==0){
            this.stopProcessorTimer();
            isHipo4Event = true;
            JFileChooser fc = new JFileChooser();
            fc.setCurrentDirectory(null);
            CalUtility.checkFile("merged.hipo");
            String outputFile="merged.hipo";
            
            fc.setMultiSelectionEnabled(true); // <--- Enable multiple file selection
            int returnVal = fc.showOpenDialog(this);
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                File[] selectedFiles = fc.getSelectedFiles(); // <--- Get all selected files
                String[] fileNames = new String[selectedFiles.length];
                int f=0;
                HipoReader read = new HipoReader();
                read.open(selectedFiles[0].getAbsolutePath());
                HipoDataSync  writer = new HipoDataSync(read.getSchemaFactory());
                ...
                for (File file : selectedFiles) {
                    ...
                    while(source.hasEvent()) {
                        DataEvent hev = source.getNextEvent();
                        
                        if(hev.hasBank("REC::Particle") && 
                                hev.getBank("REC::Particle").getInt("pid",0)==11) {
                            writer.writeEvent(hev);
                            count++;
                            if(count%10000==1) System.out.println("count "+count);
                        }
                    }
                }
                System.out.println("MERGED FILE WITH "+count+" EVENTS....");
                writer.close();
                HipoDataSource source = new HipoDataSource();
                source.open("merged.hipo");
                
                // This is where a real application would open the file.
                this.dataProcessor.setSource(source);
                this.setDataFile("merged.hipo");
                this.setDataFiles(fileNames);
                statusLabel.setText(dataProcessor.getStatusString());
                mediaNext.setEnabled(true);
                mediaPrev.setEnabled(true);
                mediaPlay.setEnabled(true);
